...
from legged_gym.envs.m20_stand.m20_handstand_Config import M20HandstandCfg,M20HandstandCfgPPO
class M20stand(BaseTask):
    def __init__(self, cfg: M20HandstandCfg, sim_params, physics_engine, sim_device, headless):
    ...
    def compute_observations(self):
        ...
        self.obs_buf = torch.cat((  
                                    ...
                                    (dof_pos_no_wheel - default_dof_pos_no_wheel) * self.obs_scales.dof_pos,#12 关节位置
                                    dof_vel_no_wheel * self.obs_scales.dof_vel, #12 关节速度
    ...
    def _get_noise_scale_vec(self, cfg):
        ...
        noise_vec[12:24] = noise_scales.dof_pos * noise_level * self.obs_scales.dof_pos
        noise_vec[24:36] = noise_scales.dof_vel * noise_level * self.obs_scales.dof_vel
        noise_vec[36:40] = noise_scales.wheel_vel * noise_level * self.obs_scales.wheel_vel
        noise_vec[40:56] = 0. # previous actions
        if self.cfg.terrain.measure_heights:
    ...
    def _reward_dof_acc(self):
        ...
        dof_acc = self.last_dof_vel - self.dof_vel
        dof_acc[:, self.wheel_indices] = 0.0 # do not penalize wheel accelerations
    ...
    def _reward_dof_acc_wheel(self):
        # Penalize dof accelerations
        dof_acc = self.last_dof_vel - self.dof_vel
        wheel_dof_acc = dof_acc[:, self.wheel_indices]
        return torch.sum(torch.square(wheel_dof_acc), dim=1)
    ...
    def _reward_default_pos(self):
        ...
        pos_error = torch.abs(self.dof_pos - self.descire_joint_pos)
    ...
    def _reward_hip_default_pos(self):
        pos_error = torch.abs(self.dof_pos - self.descire_joint_pos)
        hip_indices = [0,4,8,12]
        hip_error = pos_error[:, hip_indices]
        return torch.sum(hip_error, dim=1)

    def _reward_front_wheels_vel(self):
        front_wheel_indices = [3,11]
        front_wheel_vel = self.dof_vel[:, front_wheel_indices]
        contact = torch.norm(self.contact_forces[:, self.feet_name_reward_indices , :], dim=-1) > 1.0  # 1 contact, 0 no contact
        reward = front_wheel_vel * (~contact).float()  # only reward when not in contact
        return torch.sum(torch.square(reward), dim=1)