...
class M20JUMPCfg( LeggedRobotCfg ):
    class env:
        # change the observation dim
        frame_stack = 10 #action stack
        c_frame_stack = 3 #critic 网络的堆叠帧数
        num_single_obs = 53 #这个是传感器可以获得到的信息
        num_observations = int(frame_stack * num_single_obs) # 10帧正常的观测
        single_num_privileged_obs = 72  #不平衡的观测，包含了特权信息，正常传感器获得不到的信息
...
class M20JUMPPPO(LeggedRobotCfgPPO):
    seed = 1
    ...
    class runner:
        ...
        experiment_name = 'm20_spring_jump'
        run_name = ''
        # load and resume
        resume = False
