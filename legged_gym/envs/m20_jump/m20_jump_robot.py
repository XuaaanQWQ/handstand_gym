...
from legged_gym.envs.m20_jump.m20_jump_config import M20JUMPCfg, M20JUMPPPO
def get_euler_xyz_tensor(quat):
    ...
    return euler_xyz
class M20Jump(BaseTask):
    def __init__(self, cfg: M20JUMPCfg, sim_params, physics_engine, sim_device, headless):
    ...
    def update_obs_latency_buffer(self):#????为什么这个的调用频率1000HZ
        if self.cfg.domain_rand.randomize_obs_motor_latency:
            ...
            dof_pos_no_wheel = torch.index_select(self.dof_pos, dim=1, index=keep_indices_tensor)
            default_dof_pos_no_wheel = torch.index_select(self.default_dof_pos, dim=1, index=keep_indices_tensor)
            dof_vel_no_wheel = torch.index_select(self.dof_vel, dim=1, index=keep_indices_tensor)
            wheel_vel = self.dof_vel[:, self.wheel_indices]

            q = (dof_pos_no_wheel - default_dof_pos_no_wheel) * self.obs_scales.dof_pos
            dq = dof_vel_no_wheel * self.obs_scales.dof_vel
            wdq = wheel_vel * self.obs_scales.wheel_vel

            self.obs_motor_latency_buffer[:,:,1:] = self.obs_motor_latency_buffer[:,:,:self.cfg.domain_rand.range_obs_motor_latency[1]].clone()
            self.obs_motor_latency_buffer[:,:,0] = torch.cat((q, dq, wdq), 1).clone()
    ...
    def check_jump(self):
        ...
        pos_error[:, self.wheel_indices] = 0.0 # do not penalize wheel positions
        self.recovery=torch.logical_and(self.has_jumped,torch.sum(torch.abs(pos_error),dim=1)<0.8) #落地过并且与默认关节角度的距离小于一个值就算初始化完成????
    ...
    def compute_observations(self):   
        ...
        self.privileged_obs_buf = torch.cat((
            self.commands,  # 3 控制输入 ，相位，目标速度，角速度
            (dof_pos_no_wheel - default_dof_pos_no_wheel) * self.obs_scales.dof_pos,  # 12  当前关节位置与默认关节位置之差
            dof_pos_no_wheel * self.obs_scales.dof_pos,  # 12
            dof_vel_no_wheel * self.obs_scales.dof_vel,  # 速度乘以缩放因子 12
            wheel_vel * self.obs_scales.wheel_vel, #4 轮子速度                  
            self.actions,  # 12
        ...
        if self.cfg.domain_rand.randomize_obs_motor_latency:
            ...
            self.obs_motor = torch.cat((q, dq, wdq), 1)
    ...
    def _compute_torques(self, actions):
        ...
        if control_type=="P":
            ...
            torques = self.p_gains * (actions_scaled + dof_err) + \
                self.d_gains * (vel_ref - self.dof_vel)# elif control_type=="V":
        elif control_type=="V":
    ...
    def _init_buffers(self):
        ...
        self.obs_motor_latency_buffer = torch.zeros(self.num_envs,self.num_actions * 2 -4,self.cfg.domain_rand.range_obs_motor_latency[1]+1,device=self.device)
    ...
    def _reward_before_setting(self):
        #切换到蹲姿状态之前的奖励函数
        pos_error = self.dof_pos-self.lie_joint_pos
        pos_error[:, self.wheel_indices] = 0.0 # do not penalize wheel positions
        rew = torch.exp(-torch.sum(torch.abs(pos_error)/2,dim=1))*~self.settled_after_init
        return rew
    ...
    def _reward_base_height_stance(self):
        ...
        pos_error = self.dof_pos-self.lie_joint_pos
        pos_error[:, self.wheel_indices] = 0.0 # do not penalize wheel positions
        rew += torch.exp(-torch.sum(torch.abs(pos_error)/2,dim=1))*self.has_jumped
        return rew 
    ...
    def _reward_default_pose_air(self):
        ...
        angle_diff = torch.square(pos_error).sum(dim=1)*self.was_in_flight
        return angle_diff
